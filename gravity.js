...
var textDeviation = {'tetraVertex':0,'tetraEdge':0,'cubeVertex':0,'cubeEdge':0,'octaVertex':0,'octaEdge':0,'dodecaVertex':40,'dodecaEdge':-20,'icosaVertex':40,'icosaEdge':0};
...
for(var i=0;i<linesToDraw.length;i++){
  ...
  if(linesToDraw[i].contains('Edge')){
    var lineDrawn = canvas.path(parseLine(LEFT_MARGIN,CANVAS_VERT_SIZE-BOT_MARGIN,horizDeviation,TOP_MARGIN)).attr({'stroke-width':LINE_WIDTH-2}).attr({'stroke-linecap':'round'}).attr({'stroke':colors[linesToDraw[i]]});
  }else{
  ...
  var lineLabel1 = canvas.text(horizDeviation+textDeviation[linesToDraw[i]],TOP_MARGIN-4*TEXT_VERT_SIZE,fullText1[linesToDraw[i]]).attr({'stroke':colors[linesToDraw[i]]});
  var lineLabel2 = canvas.text(horizDeviation+textDeviation[linesToDraw[i]],TOP_MARGIN-3*TEXT_VERT_SIZE,fullText2[linesToDraw[i]]).attr({'stroke':colors[linesToDraw[i]]});
  var lineLabel3 = canvas.text(horizDeviation+textDeviation[linesToDraw[i]],TOP_MARGIN-2*TEXT_VERT_SIZE,fullText3[linesToDraw[i]]).attr({'stroke':colors[linesToDraw[i]]});
  var lineLabel4 = canvas.text(horizDeviation+textDeviation[linesToDraw[i]],TOP_MARGIN-TEXT_VERT_SIZE,'from the normal').attr({'stroke':colors[linesToDraw[i]]});
}
